...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool print_status; //if true, prints out what's written in the file so far to console
    ...
    while (getline(reading, lookLine))
    {
        ...
        for (int count = 0; count < 5; count++)
        {
            ...
            if(lookLine == "") //if there's nothing to parse then nothing happens
            {
                continue;
            }
        }
...
is a number or not one of the directions) then this specific run in the while loop is skipped using continue. */
        ...
        else if (commands[0] == "2") //pen is on page
        {
            ...
            //good practice to make sure statuses are defined
            else 
            {
                bold_status = false;
                print_status = false;
...
void printFile()
{
    ...
    takeLine.clear(); //clear eof flag
    takeLine.seekg(0, ios::beg);
    //while loop until the end of the file
    while (!takeLine.eof()) //loop as long as input file isn't done
    {
        //read a line and save to printLine
        getline(takeLine, printLine); //get line from takeLine and put into printLine
        //if doesn't fulfill the checkLine function then
        if (printLine != "") //if printLine isn't a null string
        {
            cout << printLine << "\n"; //print out the line for that run through loop
        }
    }
    cout << endl << endl; //two new lines after
    takeLine.close(); //close the istream
}

//checks if the value of commands[2] is a digit
bool checkDistance(string move)
{
    //for each char in the commands[2], check
    for (int i = 0; i< move.length(); i++)
    {
        if(!isdigit(move[i])) //if not a digit, returns false
        {
            return false;
        }
    }
